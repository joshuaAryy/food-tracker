...
| ENV-STAGE-001 | Railway staging provenance | RESOLVED for API/staging evidence. The first upload created `b0d04d9b-c9d2-414f-94d3-647007339f59` but stayed `INITIALIZING` without a build. A clean tracked `git archive` of exact candidate `14c0472c1aeb59abe75e4f0cf8507dd70e74a98a` was retried against the existing `food-tracker-staging-api` staging service, creating `f522fae3-825e-425c-810d-344fc850db6d`. Railway built the workspace, ran Prisma generation and API/shared build, ran readiness migrations with no pending migrations, and marked the deployment `SUCCESS`; image digest is `sha256:280dc90401530e212496d65d4b76a649653cb863fc36067f8570314a95d89f32`. Railway upload metadata has no commit hash, so source provenance is the exact archive command/candidate SHA plus deployment ID rather than an inferred Git field. After more than ten minutes idle, Railway logged `Stopping Container`; the first post-sleep `/health/ready` request returned 200 in 5.490 seconds and the deployment recovered to `SUCCESS`. | Staging source provenance and API cold-start behavior are evidenced for deployment `f522fae3-825e-425c-810d-344fc850db6d`. No production service or data was touched. The current Simulator bundle targets local API, so this does not claim a staging-target UI cold launch. | Preserve the deployment ID/timing in closeout; keep full staging-target Simulator UI cold launch as an explicitly unverified follow-up unless a staging-configured bundle is built. |
