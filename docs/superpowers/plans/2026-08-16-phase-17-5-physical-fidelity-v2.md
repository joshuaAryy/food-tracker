...
- [x] Run changed-file Prettier and `git diff --check`.
- [x] Commit only these four intentional documentation paths as `docs: establish visual QA workflow v2`.
...
- [x] Write failing tests for floating-point noise, explicit zero, null unknown, and metric-specific precision.
- [x] Run the focused Vitest tests and confirm the new assertions fail for the current implementation.
- [x] Implement the smallest helpers using the existing date-only noon-anchor approach.
- [x] Run the focused tests and confirm they pass.
- [x] Replace Phase 17.5 user-facing direct `toFixed`/raw interpolation in report, chart tooltip, selected-point, comparison, custom-range, and forecast surfaces.
...
- [x] Add failing assertions for a 390pt macro center that fits value/unit without clipping, a minimum donut diameter, separated legend, and a nontrivial Protein trend height.
- [x] Add failing assertions for Weight chart geometry that consumes the card surface and retains target/reference/forecast affordances.
- [x] Run focused tests and confirm the failures are caused by current compact dimensions/formatting.
- [x] Implement component-specific dimensions mapped to Figma `338:720` and `338:605`; do not shrink typography as the primary fix.
- [x] Apply the locked macro colors (`#C9242D`, `#33B866`, `#FFAD8F`) and semantic range/reference treatment using existing chart primitives.
...
- [x] Add failing tests proving 30D selects intentional density/layout behavior while preserving all four logging states.
- [x] Add failing tests for hydration vessel presentation using non-black blue-system colors.
- [x] Run focused tests and confirm failures.
- [x] Implement Figma-sized heatmap/meal-coverage/period-pattern cards for the two period classes without changing classification semantics.
- [x] Implement durable local vessel visuals with blue outlines, fills, spacing, and target treatment; keep water CRUD untouched.
...
- [x] Add failing assertions for the range chart height, reference band/line, average label, related metric, and contributor hierarchy.
- [x] Run the focused test and confirm the failure.
- [x] Implement the final-node composition with existing chart/reference/card components and semantic glow/range treatment.
- [x] Ensure related metric naming/content is derived from the selected nutrient metadata, not hard-coded Vitamin C copy.
- [x] Run focused tests and refactor while green.
