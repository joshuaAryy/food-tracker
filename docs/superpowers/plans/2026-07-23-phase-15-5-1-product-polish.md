...
- [ ] Once the storage gate passes, run `corepack pnpm --filter @food-tracker/mobile ios:dev-build:device` with bundle identifier `ca.joshuaaryeetey.foodtracker` and Apple team selected in Xcode, then use the existing simulator workflow.
