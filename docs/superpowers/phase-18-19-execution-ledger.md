...
- Commits through `b495c51` record the contribution, documentation, locale,
  release-audit, and configurable semantic-model checkpoints; the branch is
  synchronized with its origin branch.
- Focused benchmark/retrieval tests pass: 4 files, 140 tests, including
...
- Fresh focused verification for code checkpoint `b495c51`
  under Node `v22.23.0` and pnpm
  `10.34.3` passes `lint`, `typecheck`, focused retrieval tests (4 files, 140
