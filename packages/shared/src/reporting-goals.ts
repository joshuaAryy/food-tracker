...
export type ReportingGoalDirection =
  (typeof REPORTING_GOAL_DIRECTIONS)[number];
...
const DEFAULT_EXTENDED_GOALS: Record<
  ExtendedNutrientKey,
  DefaultExtendedGoal
> = {
...
function explicitOrDerived(
  ...
  if (positiveFinite(derived))
    return goal(derived, unit, direction, 'derived');
...
export function resolveReportingGoals(
  ...
  const derivedFiber =
    calories === null ? null : roundToTenth(Math.max(1, (calories / 1000) * 14));
  const derivedSugar =
    calories === null
      ? null
      : roundToTenth(Math.max(1, (calories * 0.1) / 4));
  ...
  const goals: ReportingGoals = {
    ...
    sodium:
      positiveFinite(inputs.limitSodiumMg)
