...
const override = process.env.NOTIFICATION_ACCEPTANCE_TIME_OVERRIDE_ENABLED === 'true';
...
const now = atIndex >= 0 && args[atIndex + 1] !== undefined
  ? new Date(args[atIndex + 1] as string)
  : new Date();

if (override && (userIndex < 0 || args[userIndex + 1] === undefined)) {
  throw new Error('A single acceptance Firebase UID is required when clock override is enabled.');
}
...
const acceptanceUserId = acceptanceFirebaseUid === undefined
  ? undefined
  : (await prisma.user.findFirst({ where: { firebaseUid: acceptanceFirebaseUid }, select: { id: true } }))?.id;
if (override && acceptanceUserId === undefined) {
  throw new Error('The acceptance Firebase UID must belong to one staging user.');
}
...
console.log(JSON.stringify({ evaluated: result.evaluated, claimed: result.claimed, dryRun }));
