...
export function interpretAnalyticsReference(
  ...
  if (reference.kind === 'range') {
    if (average < reference.lower) {
      return {
        kind: 'below_range',
        message: 'Recorded average is below the configured range.',
      };
    }
    if (average > reference.upper) {
      return {
        kind: 'above_range',
        message: 'Recorded average is above the configured range.',
      };
    }
    return {
      kind: 'within_range',
  ...
  if (reference.kind === 'minimum') {
    return average < reference.value
      ? {
          kind: 'below_minimum',
          message: 'Recorded average is below the configured minimum.',
        }
      : {
          kind: 'meets_minimum',
  ...
  return average > reference.value
    ? {
        kind: 'above_limit',
        message: 'Recorded average is above the configured limit.',
      }
    : {
        kind: 'within_limit',
        message: 'Recorded average is within the configured limit.',
      };
}
